<template>
    <div class="pagination">
        ...
        <button v-for="(item, index) in startNumAndEndNum.end" :key="index" v-show="item >= startNumAndEndNum.start"
            @click="$emit('getPageNo', item)" :class="{ active: pageNo === item }">{{
                    item
            }}</button>

        <button v-show="startNumAndEndNum.end < totalPage - 1">···</button>
        <button v-show="startNumAndEndNum.end < totalPage" @click="$emit('getPageNo', totalPage)">{{ totalPage
        }}</button>
    ...
    computed: {
        ...
        startNumAndEndNum() {
            ...
            if (continues > totalPage) {
                ...
                // 把不正常的现象【start数字出现 0|负数】纠正 也能（if（start<1））
                if (pageNo < 3) {
                    start = 1
                    end = continues;
                };
                // 把出现不正常的现象[end数字大于总页码]纠正 也能（if（end>totalPage））
