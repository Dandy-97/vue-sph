...
export default {
  ...
  methods: {
    ...
    async addshopcar() {
      ...
      this.$store.dispatch("AddorRemoveshopcar", { skuId: this.$route.params.skuid, skuNum: this.skuNum })
      上面这行代码说白了：调用仓库中的AddorRemoveshopcar,这个方法加上asyc,返回一定是一个Promise
      要么成功  |  要么失败 
      */
      // let res = await this.$store.dispatch("AddorRemoveshopcar", { skuId: this.$route.params.skuid, skuNum: this.skuNum });
      try {
        // 成功
        await this.$store.dispatch("AddorRemoveshopcar", { skuId: this.$route.params.skuid, skuNum: this.skuNum });
        // 路由跳转
