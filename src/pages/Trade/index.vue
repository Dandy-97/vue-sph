<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="orderList in orderInfo.detailArrayList" :key="orderList.skuId">
          <li>
            <img :src="orderList.imgUrl" alt="" style="width:100px;height:100px">
          </li>
          <li>
            <p>{{ orderList.skuName }}</p>
            <h4>7天无理由退货</h4>
          </li>
          <li>
            <h3>￥{{ orderList.orderPrice }}.00</h3>
          </li>
  ...
  methods: {
    ...
    async submitOrder() {
      ...
      if (res.code === 200) {
        ...
        // 路由跳转 + 路由传参 query
        this.$router.push({ name: 'pay', query: { orderId: this.orderId } }) // query传参对象写法
        // this.$router.push('/pay?orderId='+this.orderId); // query传参字符串写法
        // this.$router.push(`/pay?orderId=${this.orderId}`)  // query传参模板字符串写法
      } else {
        alert(res.message)
      }
    }
