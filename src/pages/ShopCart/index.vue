  ...
  <div class="cart">
    ...
    <div class="cart-tool">
      ...
      <div class="money-box">
        ...
        <div class="sumbtn">
          <a class="sum-btn" @click="$router.push({ name: 'trade' })" style="cursor:pointer">结算</a>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    isAllCheck() {
      // every遍历数组原理，只要全部元素isChecked,属性都为1=>真true,只要有一个不是1=====>假fa1se、
  ...
  methods: {
    ...
    handler: throttle(async function (type, disNum, cart) {
      ...
      switch (type) {
        ...
        case "minus": disNum = cart.skuNum > 1 ? -1 : 0;
          break;
        case "change": if (isNaN(disNum) || disNum < 1) {
          // 用户输入进来的最终量，非法的（带有汉字字母），需带给服务器数字
          disNum = 0;
        } else {
      ...
      try {
        // 代表的是修改成功
        await this.$store.dispatch('AddorRemoveshopcar', { skuId: cart.skuId, skuNum: disNum });
        // 再一次获取服务器最新的数据进行展示
        this.getData();
      } catch (error) {
        alert(error.message);
    ...
    // 修改全部产品选中状态
    async changeAllCheckedCart(event) {
      try {
        let isChecked = event.target.checked ? '1' : '0';
        // 派发action
