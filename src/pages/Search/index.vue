    ...
    <div class="main">
      <div class="py-container">
        ...
        <div class="details clearfix">
            ...
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{ active: inOne }" @click="changeOrder('1')">
                  <a>综合<span v-show="inOne"><i v-show="isDesc">⬇</i><i v-show="isAsc">⬆</i></span></a>
                </li>
                <li :class="{ active: inTow }" @click="changeOrder('2')">
                  <a>价格<span v-show="inTow"><i v-show="isDesc">⬇</i><i v-show="isAsc">⬆</i></span></a>
                </li>
              </ul>
            </div>
          </div>
          ...
          <!-- 全局组件分页器 -->
          <Pangination :pageNo="searchParams.pageNo" :pageSize="searchParams.pageSize" :total="getTotal" :continues="5"
            @getPageNo="getPageNo">
          </Pangination>
        </div>
      </div>
...
export default {
  ...
  beforeMount() {
    //在发请求之前，把接口需要传递参数，进行整理（在给服务器发清求之前，把参数整理好，服务器就会返回查询的数据）
    Object.assign(this.searchParams, this.$route.query, this.$route.params);
  },
  //组件挂载完毕执行一次【仅仅执行一次】
  mounted() {
